...
var arr = [];
localStorage.setItem('mm','12345');
console.log(localStorage.getItem('mm'));
fetchAPI.then(api=>{return api.filter(el=>el.id == greetingValue)})
.then(res=>{
    ...
    res.map(el=>{
        ...
        productDetail.innerHTML = `<div class="row"><div class="col-md-8 col-sm-6 col-12 position-relative"><div class="row justify-content-between"><div class="col-6"><span class="px-3"><i class="fa fa-heart fa-lg text-danger"></i><i class="fa px-1 fa-share-nodes fa-lg text-primary"></i></span></div><div class="col-6 mt-1" dir="rtl"><h4>${el.title}</h4></div></div><div class="row my-3" dir="rtl"><h5> ${el.category}</h5></div><div class="row my-4 ps-3"><p class="text-start">${el.description}</p></div><div class="row fs-5 ps-3 justify-content-between"><div class="col"><span class="mx-1">${el.rating.rate}</span><i class="fa fa-star text-warning"></i></div><div class="col" dir="rtl"><i class="fa fa-user-edit"></i><span class="mx-1">${el.rating.count}</span></div></div><div class="product-price d-flex justify-content-between position-absolute w-100 align-items-end mt-5 me-2" dir="rtl"><div class="col"><a href="cart.html" id="productSubmit" class="btn btn-success fw-bold btn-custom-success">افزودن به سبد خرید</a></div><div class="col ps-2 " dir="ltr"><div class="d-flex justify-content-between align-items-end"><span class="pt-1 px-2 rounded-3"><del class="del-price">${price} $</del><span class="reduction-box text-white ms-4 mt-5 fw-bold px-1">50%</span><div class="pt-2 h4">${el.price} $</div></span></div></div></div></div><div class="col-md-4 col-sm-6 col-12"><div class="w-100 py-4 px-5"><img class="img-fluid productDetail-image" alt="product image" src="${el.image}"></div></div></div>`;
